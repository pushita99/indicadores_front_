		...
		<div class="col-12">
			<div class="card">
				...
				<DataTable ref="dt" :value="indicators" v-model:selection="selectedIndicators" dataKey="id"
					...
					<Column field="measurementunitId" header="Unidad de medida" :sortable="true" headerStyle="width:14%; min-width:10rem;">
						<template #body="slotProps">
							<span class="p-column-title">Unidad de medida</span>
							<span :class="'product-badge status-' + (slotProps.data.measurementunitId ? slotProps.data.measurementunitId.toLowerCase() : '')">{{slotProps.data.measurementunitId}}</span>
						</template>
					</Column>
